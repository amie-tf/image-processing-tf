...
resource "google_cloudfunctions_function" "function" {
  name        = "${local.function_name}-${random_string.bucket_suffix.result}"
  ...
  trigger_http          = true
  entry_point           = "detect_image"
  service_account_email = google_service_account.images_worker.email
...
resource "google_storage_bucket" "source" {
  name = "${var.project}-source-${random_string.bucket_suffix.result}"
}
